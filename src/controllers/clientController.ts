...
export async function create(req: Request, res: Response, next: NextFunction) {
  try {
    const { name, email, company, phone, avatarUrl } = req.body;
    if (!name) throw new AppError('Name is required', 400);
    const result = await pool.query(
      `INSERT INTO clients (org_id, name, email, company, phone, avatar_url)
       VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`,
      [req.user!.orgId, name, email, company, phone, avatarUrl]
    );
...
export async function update(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const result = await pool.query(
      ...
      [name, email, company, phone, avatarUrl, req.params.id, req.user!.orgId]
    );
    if (!result.rows[0]) throw new AppError('Client not found', 404);
